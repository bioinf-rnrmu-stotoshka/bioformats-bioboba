...
Использование:
    python run_vcf.py <путь_к_vcf> [CHROM START END]
...
from vcf_reader import VcfReader  # Предполагается, что ваш VcfReader сохранён в vcf_reader.py
...
def main() -> None:
    ...
    Выполняет следующие задачи в соответствии с техническим заданием:
        1. Выводит мета-заголовки VCF-файла (строки, начинающиеся с '##').
        2. Группирует и отображает информацию по ключевым секциям заголовка:
           ##INFO, ##FILTER, ##FORMAT, ##contig.
        3. Подсчитывает общее количество вариантов в файле.
        4. Формирует и выводит статистику по хромосомам (регионам) с использованием pandas.
        5. (Опционально) Фильтрует и отображает варианты в заданном геномном регионе.
